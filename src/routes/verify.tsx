...
function VerifyPage() {
  ...
  // The facial scan animation is a local UI simulation only. The account is
  // NOT marked verified until the backend confirms the biometric match.
  // Never set `verified: true` purely from client-side state.
  const confirmBiometricMatch = async () => {
    try {
      const s = getSession();
      const res = await apiRequest("/auth/verify-biometric/", "POST", {
        nin: s?.nin || "",
      });

      if (!res?.verified) {
        throw new Error("Biometric match could not be confirmed.");
      }
      ...
      setScanning(false);
      setProgress(0);
      setError(err.message || "Facial match failed. Please try again in good lighting.");
    }
  };
